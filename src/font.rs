// 8x8 bitmap font extracted from /usr/share/misc/pcvtfonts/koi8-r-8x08 (public domain)
// Each entry is 8 rows; each byte: MSB = leftmost pixel
...
pub fn draw_char(buf: &mut [u32], stride: usize, x: usize, y: usize, ch: char, fg: u32, bg: u32) {
    ...
    for row in 0..8usize {
        ...
        for col in 0..8usize {
            let px = x + col;
            let py = y + row;
            let pixel = if (bits >> (7 - col)) & 1 == 1 { fg } else { bg };
            let i = py * stride + px;
            if i < buf.len() {
                buf[i] = pixel;
            }
...
pub fn draw_str(buf: &mut [u32], stride: usize, x: usize, y: usize, s: &str, fg: u32, bg: u32) {
    for (i, ch) in s.chars().enumerate() {
        draw_char(buf, stride, x + i * 8, y, ch, fg, bg);
    }
}
